...
def get_tasks():
    ...
    res = _get_neo4j().query(get_task_nodes)
...
def get_task_nodes(task_id):
    ...
    res = _get_neo4j().query(get_task_nodes, parameters)
    task_dict = convert_json(res)
    return task_dict
...
def get_similar_tasks(query_task, num_res=3):
    ...
    query_embedding = torch.tensor(_get_embedding_model().encode(str(query_task))).view(1, -1).to(DEVICE)
    ...
    logger.debug("num_res: %s", num_res)
    ...
    logger.info("get_similar_tasks completed in %.2fs", time.time()-start_time)
    similar_item_dict = []
